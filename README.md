...
`noImplicitThis` and `useUnknownInCatchVariables` are already on. `tsconfig.json` spells out the remaining strictness flags in the order they are worth turning on next (`noImplicitAny`, then `strictNullChecks`).
